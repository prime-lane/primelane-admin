...
export const useUserRideStats = (id: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<UserRideStats>(
        e.ANALYTICS.USER_RIDE_STATS(id),
      )
      return response.data
    },
    enabled: !!id,
...
export const useCustomerWallet = (userId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<{ data: Wallet }>(
        `${e.WALLETS.WALLET}`,
      )
...
export const useCustomerTransactions = (
  params: UseCustomerTransactionsParams,
) => {
  return useQuery({
