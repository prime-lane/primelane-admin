...
import { useRoles, useUpdateAdmin } from '../api/use-admins'
...
export const EditAdminModal = ({ open, onClose, admin }: EditAdminModalProps) => {
  ...
  useEffect(() => {
    if (open && roles) {
      const matchedRole = roles.find((r) => r.name === admin.role_name)
      reset({
        first_name: admin.first_name,
        last_name: admin.last_name,
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
      <DialogTitle>
        <span className="text-2xl font-medium font-sans">Edit admin</span>
      </DialogTitle>
      <DialogContent>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      sx={{ color: `${colors.neutral[500]} !important` }}
                    >
                      {isLoadingRoles ? (
