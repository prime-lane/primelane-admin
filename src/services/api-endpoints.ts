...
export const API_ENDPOINTS = {
  AUTH: {
    ...
    ADMIN_OTP: (action: 'verify-phone' | 'login' = 'login') =>
      `/auth/admin-otp/${action}`,
    VERIFY_OTP: (action: 'verify-phone' | 'login') =>
      `/auth/verify-otp/${action}`,
    CONFIRM_OTP: (action: 'reset-password' | 'change-email' | 'change-phone') =>
      `/auth/confirm-otp/${action}`,
    RESEND_OTP: (action: 'verify-phone' | 'login') =>
      `/auth/resend-otp/${action}`,
    GOOGLE: {
